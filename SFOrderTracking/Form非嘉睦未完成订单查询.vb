Public Class Form非嘉睦未完成订单查询
    ...
    Private Sub DataGridView1_CellContentDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles DataGridView1.CellContentDoubleClick
        ...
        If ((DataGridView1.Columns(e.ColumnIndex).Name = "DrawNo") And (e.RowIndex >= 0)) Then
            Shell(SFOrderBaseNetConnect, vbHide)
    ...
    Private Sub ComboBox1_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ComboBox1.SelectedIndexChanged
        ...
        SQLstring = "Select SFOrder '赛峰单号',DWGInfo '图号信息',DrawNo '图号',PartName '零件名称',CustOrder '客户单号',Qty '数量',CDate '下单日期',DDate '交货日期',OrderType '订单类型',CustCode '客户代码',Status '状态',RecordTime '记录时间',FollowNote '跟单备注' from StatusInfo where (CustCode<>'C001' and CustCode<>'C002') and ((Status not in ('完成入库','取消生产单','已终检') or isnull(Status,'')='') and (Status like " & "'%" & ComboBox1.SelectedItem.ToString & "-%'" & "))"
        cn.Open(CNsfmdb)
        rs.Open(SQLstring, cn, 1, 1)
