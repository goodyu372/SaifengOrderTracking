Public Class Form按客户查询未完成零件状况
    Private Sub TextBox1_KeyDown(sender As Object, e As KeyEventArgs) Handles TextBox1.KeyDown
        If (e.KeyCode = 13) Then
            ...
            SQLstring = "Select * from SFOrderBase where CustCode=" & "'" & TextBox1.Text & "'"
            cn.Open(CNsfmdb)
            ...
            End If
            rs.Close()
            'SQLstring = "Select SFOrder '赛峰单号',DWGInfo '图号信息',DrawNo '图号',CustOrder '客户单号',Qty '数量',CDate '下单日期',DDate '交货日期',OrderType '订单类型',CustCode '客户代码',Status '状态',RecordTime '记录时间',FollowNote '跟单备注' from StatusInfo where ((Status not in (select Status where Status='完成入库' or Status='取消生产单' or Status='已终检')) and (CustCode=" & "'" & TextBox1.Text & "'))"
            SQLstring = "Select SFOrder '赛峰单号',DWGInfo '图号信息',DrawNo '图号',PartName '零件名称',CustOrder '客户单号',Qty '数量',CDate '下单日期',DDate '交货日期',OrderType '订单类型',CustCode '客户代码',Status '状态',RecordTime '记录时间',FollowNote '跟单备注' from StatusInfo where not((CHARINDEX('完成入库',Status)>0) or (CHARINDEX('取消生产单',Status)>0) or (CHARINDEX('已终检',Status)>0)) and (CustCode=" & "'" & TextBox1.Text & "')"
    ...
    Private Sub Form17_FormClosed(sender As Object, e As FormClosedEventArgs) Handles Me.FormClosed
        Form订单查询.Show()
    End Sub
    Private Sub Form17_Resize(sender As Object, e As EventArgs) Handles Me.Resize
