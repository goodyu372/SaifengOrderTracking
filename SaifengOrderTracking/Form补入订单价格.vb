Public Class Form补入订单价格
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        For i = 0 To DataGridView1.Rows.Count - 1
            ...
            Else
                SQLstring = "Select ID,UnitP,Qty from SFOrderBase where ID=" & "'" & DataGridView1.Item("ID", i).Value & "'"
            End If
            ...
            If UserName = "admin" Then
                ...
                rs.Fields("DWGInfo").Value = rs("CustCode").Value & DataGridView1.Item("DrawNo", i).Value & rs("SFOrder").Value
                rs.Fields("CustDWG").Value = rs("CustCode").Value & DataGridView1.Item("DrawNo", i).Value
            End If
            SQLOutput = "Select ID, SFOrder, DrawNo,UnitP from OutputRecord where SFOrder =" & "'" & DataGridView1.Item("SFOrder", i).Value & "'" & " and DrawNo=" & "'" & DataGridView1.Item("DrawNo", i).Value & "'"
            rsOutput.Open(SQLOutput, cn, 1, 3)
            If rsOutput.RecordCount > 0 Then
                rsOutput.MoveFirst()
                Do While rsOutput.EOF = False
                    rsOutput.Fields("UnitP").Value = DataGridView1.Item("UnitP", i).Value
                    rsOutput.Update()
                    rsOutput.MoveNext()
                Loop
            End If
